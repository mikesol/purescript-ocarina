...
for x in A.split('\n'):
  ...
  #print(' '.join(['  '+a[0]+' = '+a[0][0].upper()+a[0][1:]]+a[1:]).replace('-> audio -> engine audio','').replace('::','').replace('->','') )
  #print('  '+a[0]+' a b c = '+a[0]+'_ a b c')
  print('  '+a[0]+' = safeToFFI '+a[0]+'_')
  #print('foreign import '+a[0]+'_ :: Int')
  #z = [y for y in x.split(' ') if y != '']
  #print('foreign import ' + (' '.join([z[0]+'_']+z[1:])).replace(' -> audio', " -> FFIAudio'").replace(' -> engine audio', " -> Effect FFIAudio'").replace('AudioParameter',"FFIAudioParameter'") )