...
ARITY ={'Allpass':3, 'Bandpass':3, 'Constant':1, 'Convolver':2, 'Delay':2, 'Dup':-1, 'DynamicsCompressor':6, 'Gain':2, 'Highpass':3, 'Highshelf':3, 'LoopBuf':4, 'Lowpass':3, 'Lowshelf':3, 'Microphone':1, 'Notch':3, 'Peaking':4, 'PeriodicOsc':2, 'PlayBuf':3, 'Recorder':2, 'SawtoothOsc':1, 'SinOsc':1, 'Speaker':1, 'SquareOsc':1, 'StereoPanner':2, 'TriangleOsc':1, 'WaveShaper':3}
SV={'Allpass':(0,1), 'Bandpass':(0,1), 'Constant':(0,), 'Convolver':(), 'Delay':(0,), 'Dup':(), 'DynamicsCompressor':(0,1,2,3,4), 'Gain':(0,), 'Highpass':(0,1), 'Highshelf':(0,1), 'LoopBuf':(1,), 'Lowpass':(0,1), 'Lowshelf':(0,1), 'Microphone':(), 'Notch':(0,1), 'Peaking':(0,1,2), 'PeriodicOsc':(1,), 'PlayBuf':(2,), 'Recorder':(), 'SawtoothOsc':(0,), 'SinOsc':(0,), 'Speaker':(), 'SquareOsc':(0,), 'StereoPanner':(0,), 'TriangleOsc':(0,), 'WaveShaper':()}
...
def change_instructions(s):
  if len(SV[s]) == 0:
    return f'''instance changeInstructions{s} :: ChangeInstructions (CTOR.{s} {' '.join(args[:ARITY[s]])}) where
  changeInstructions _ _ _ = Nothing'''
  return f'''instance changeInstructions{s} :: ({', '.join(['SetterVal '+y for x,y in enumerate(args[:ARITY[s]]) if x in SV[s] ])}) => ChangeInstructions (CTOR.{s} {' '.join(args[:ARITY[s]])}) where
  changeInstructions idx (CTOR.{s} {' '.join([y if x in SV[s] else '_' for x,y in enumerate(args[:ARITY[s]])])}) = case _ of
    A{s} {' '.join(['v_'+y+'@(AudioParameter v_'+y+"')" for x,y in enumerate(args[:ARITY[s]]) if x in SV[s] ])} ->
      let
{chr(10).join([f"""        s_{y} = setterVal {y}

        {y}_iv' = s_{y} v_{y}

        {y}_Changes = let AudioParameter {y}_iv = {y}_iv' in if {y}_iv.param == v_{y}'.param then [] else [ {STR[s]['_'+str(x)]} idx {y}_iv.param {y}_iv.timeOffset {y}_iv.transition ]""" for x,y in enumerate(args[:ARITY[s]]) if x in SV[s]] ) }
      in
        Just
          $ ({' <> '.join([y+'_Changes' for x,y in enumerate(args[:ARITY[s]]) if x in SV[s] ])})
