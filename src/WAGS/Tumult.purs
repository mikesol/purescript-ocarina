...
makeTumult
  :: forall n terminus inputs scene graph
   . Pos n
  => Create scene () graph
...
reconcileTumult new old = go primus secondus Set.empty
  ...
  go l0@(vA@(MakeAudioWorkletNode ptr0 valA0) : rest0) l1@(MakeAudioWorkletNode ptr1 _ : rest1) set
    ...
    | otherwise =
        ...
        in
          go rest0 rest1 $ Set.union (Set.fromFoldable $ (identity :: Array ~> Array) $ map fn $ Object.toUnfoldable (unwrap valA0).parameterData) set
  ...
  go l0 (MakeSubgraphWithDeferredScene ptr1 : rest1) set =
    go l0 rest1 (Set.insert (DestroyUnit ptr1) set)
  go l0@(vA@(MakeTumult ptr0 _) : rest0) l1@(MakeTumult ptr1 _ : rest1) set
    | ptr0 < ptr1 = go rest0 l1 (Set.insert vA set)
    | ptr1 < ptr0 = go l0 rest1 (Set.insert (DestroyUnit ptr1) set)
    | otherwise = go rest0 rest1 set
  go (vA@(MakeTumult _ _) : rest0) l1 set =
    go rest0 l1 (Set.insert vA set)
  go l0 (MakeTumult ptr1 _ : rest1) set =
    go l0 rest1 (Set.insert (DestroyUnit ptr1) set)
  ...
  go (SetTumult x0 y0 : rest0) l1 set = go rest0 l1 $ Set.insert (SetTumult x0 y0) set
  go l0 (SetTumult _ _ : rest1) set = go l0 rest1 set