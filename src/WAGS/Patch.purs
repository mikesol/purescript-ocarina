...
data MakeAudioWorkletNode
  ...
  (processorOptions :: Row Type) = MakeAudioWorkletNode (Proxy ptr) (Proxy sym)
...
data MakeInput (ptr :: Symbol) (terminus :: Symbol) = MakeInput (Proxy ptr)
  (Proxy terminus)
...
data MakeWaveShaper (ptr :: Symbol) (oversample :: Type) = MakeWaveShaper
  (Proxy ptr)
  oversample
...
instance doCreateMakeAudioWorkletNode ::
  DoCreate ptr
    ( AU.TAudioWorkletNode sym numberOfInputs numberOfOutputs outputChannelCount
        parameterData
        processorOptions
    )
    ( MakeAudioWorkletNode ptr sym numberOfInputs numberOfOutputs
        outputChannelCount
        parameterData
        processorOptions
    )
...
instance doCreateMakeDynamicsCompressor ::
  DoCreate ptr AU.TDynamicsCompressor (MakeDynamicsCompressor ptr)
...
instance doCreateMakeInput ::
  DoCreate ptr (AU.TInput input) (MakeInput ptr input)
...
instance doCreateMediaElement ::
  DoCreate ptr AU.TMediaElement (MakeMediaElement ptr)

instance doCreateMakeMicrophone ::
  DoCreate "microphone" AU.TMicrophone MakeMicrophone
...
instance doCreateMakePeriodicOsc ::
  DoCreate ptr AU.TPeriodicOsc (MakePeriodicOsc ptr)
...
instance doCreateMakeSawtoothOsc ::
  DoCreate ptr AU.TSawtoothOsc (MakeSawtoothOsc ptr)
...
instance doCreateMakeStereoPanner ::
  DoCreate ptr AU.TStereoPanner (MakeStereoPanner ptr)

instance doCreateMakeSubgraph ::
  DoCreate ptr (AU.TSubgraph arity terminus inputs env) (MakeSubgraph ptr)

instance doCreateMakeTriangleOsc ::
  DoCreate ptr AU.TTriangleOsc (MakeTriangleOsc ptr)

instance doCreateMakeTumult ::
  DoCreate ptr (AU.TTumult arity terminus inputs) (MakeTumult ptr)

instance doCreateMakeWaveShaper ::
  DoCreate ptr (AU.TWaveShaper overdrive) (MakeWaveShaper ptr overdrive)
...
instance toGraphEffectsMakeAllpass ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeAllpass ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeAllpass { id, freq: paramize 440.0, q: paramize 1.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeAnalyser ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeAnalyser ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i ---------------------------------- can we make an analyser without assets?
          ...
          { instructions = i.instructions <>
              [ makeAnalyser { id, cb: AnalyserNodeCb \_ -> pure (pure unit) } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)
...
instance toGraphEffectsMakeAudioWorkletNode ::
  ...
  ToGraphEffects ( MakeAudioWorkletNode ptr sym numberOfInputs numberOfOutputs
        outputChannelCount
        parameterData
...
instance toGraphEffectsMakeBandpass ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeBandpass ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeBandpass { id, freq: paramize 440.0, q: paramize 1.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeConstant ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeConstant ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeConstant { id, onOff: onOffIze _off, offset: paramize 0.0 }
              ]
          }
      )
    where
...
instance toGraphEffectsMakeConvolver ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeConvolver ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
...
instance toGraphEffectsMakeDelay ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeDelay ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeDelay { id, delayTime: paramize 1.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeDynamicsCompressor ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeDynamicsCompressor ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
...
instance toGraphEffectsMakeGain ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeGain ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeGain { id, gain: paramize 0.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeHighpass ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeHighpass ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeHighpass { id, freq: paramize 440.0, q: paramize 1.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeHighshelf ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeHighshelf ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeHighshelf { id, freq: paramize 440.0, gain: paramize 0.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)
...
instance toGraphEffectsMakeInput ::
  ...
  ToGraphEffects (MakeInput ptr sym /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
...
instance toGraphEffectsMakeLoopBuf ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeLoopBuf ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeLoopBufWithDeferredBuffer { id } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeLowpass ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeLowpass ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeLowpass { id, freq: paramize 440.0, q: paramize 1.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeLowshelf ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeLowshelf ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeLowshelf { id, freq: paramize 440.0, gain: paramize 0.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeMediaElement ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeMediaElement ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ maybe constantNothing (makeMediaElement <<< { id, element: _ })
                  cache.mediaElement
              ]
          }
      )
...
constantNothing
  :: forall audio engine. AudioInterpret audio engine => audio -> engine
constantNothing = makeConstant
  { id: "microphone", onOff: onOffIze _on, offset: paramize 0.0 }

instance toGraphEffectsMakeMicrophone ::
  ToGraphEffects rest =>
  ToGraphEffects (MakeMicrophone /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ maybe constantNothing (makeMicrophone <<< { microphone: _ })
                  cache.microphone
              ]
          }
      )

instance toGraphEffectsMakeNotch ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeNotch ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeNotch { id, freq: paramize 440.0, q: paramize 1.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakePeaking ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakePeaking ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
...
instance toGraphEffectsMakePeriodicOsc ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakePeriodicOsc ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makePeriodicOscWithDeferredOsc { id } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakePlayBuf ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakePlayBuf ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makePlayBufWithDeferredBuffer { id } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeRecorder ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeRecorder ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeRecorder { id, cb: MediaRecorderCb \_ -> pure unit } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeSawtoothOsc ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeSawtoothOsc ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeSawtoothOsc
                  { id, onOff: onOffIze _off, freq: paramize 440.0 }
              ]
          }
      )
    where
...
instance toGraphEffectsMakeSinOsc ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeSinOsc ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeSinOsc { id, onOff: onOffIze _off, freq: paramize 440.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)

instance toGraphEffectsMakeSquareOsc ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeSquareOsc ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeSquareOsc { id, onOff: onOffIze _off, freq: paramize 440.0 }
              ]
          }
      )
    where
...
instance toGraphEffectsMakeSpeaker ::
  ( ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeSpeaker /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
...
instance toGraphEffectsMakeStereoPanner ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeStereoPanner ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeStereoPanner { id, pan: paramize 0.0 } ]
          }
      )
    where
    id = reflectSymbol (Proxy :: _ ptr)
...
instance toGraphEffectsMakeTriangleOsc ::
  ( IsSymbol ptr
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeTriangleOsc ptr /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
...
instance toGraphEffectsMakeWaveShaper ::
  ( IsSymbol ptr
  , IsOversample oversample
  , Monoid oversample
  , ToGraphEffects rest
  ) =>
  ToGraphEffects (MakeWaveShaper ptr oversample /\ rest) where
  toGraphEffects _ cache i =
    toGraphEffects (Proxy :: _ rest) cache
      ( i
          { instructions = i.instructions <>
              [ makeWaveShaper
                  { id, curve: emptyFloatArray, oversample: oversample' }
              ]
          }
      )
    where
...
unAE
  :: AE -> forall audio engine. AudioInterpret audio engine => audio -> engine
unAE (AE ae) = ae
