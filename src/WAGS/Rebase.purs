...
class Rebase iA iB where
  rebase :: forall env proof m. Monad m => Proxy iA -> Proxy iB -> FrameT env proof m iA iB Unit
...
class Rebase' plA plB rbp pA (iA :: Universe) pB (iB :: Universe) where
  rebase' :: forall env proof m. Monad m => Proxy plA -> Proxy plB -> rbp -> Proxy pA -> Proxy iA -> Proxy pB -> Proxy iB -> FrameT env proof m iA iB AFT
...
class RebaseCheck' plA plB rbp pA (iA :: Universe) pB (iB :: Universe) where
  rebaseCheck' :: forall env proof m. Monad m => Proxy plA -> Proxy plB -> rbp -> Proxy pA -> Proxy iA -> Proxy pB -> Proxy iB -> FrameT env proof m iA iB AFT
...
class RebaseCont' foundA foundB ptrA ptrB plA plB rbp pA (iA :: Universe) pB (iB :: Universe) where
  rebaseCont' :: forall env proof m. Monad m => Proxy foundA -> Proxy foundB -> Proxy ptrA -> Proxy ptrB -> Proxy plA -> Proxy plB -> rbp -> Proxy pA -> Proxy iA -> Proxy pB -> Proxy iB -> FrameT env proof m iA iB AFT

rebaseAudioUnit ::
  forall env proof ptrA ptrB m.
  Monad m =>
  BinToInt ptrA =>
  BinToInt ptrB =>
  Proxy ptrA ->
  Proxy ptrB ->
  AudioState proof env m AFT
