...
1. Engine info, which for now is just an easing algorithm. The easing algorithm is of type `Cofree ((->) Int) Int` and tells the engine how much lookahead the audio should have in milliseconds. The `(->) Int` says how much headroom there was, where positive headroom is spare time in milliseconds and negative headroom is a missed rendering deadline. This allows the algorithm to make adjustments if necessary. In this example, we have minimum lookahead of 20 that gets longer if a deadline is missed and trends towards 20 as deadlines are hit.
